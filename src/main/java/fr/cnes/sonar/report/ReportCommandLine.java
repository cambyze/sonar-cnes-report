...
public final class ReportCommandLine {
  ...
  static {
    ...
    (new File(
        org.apache.commons.io.FileUtils.getUserDirectory().getPath().concat("/.cnesreport/log")))
            .mkdirs();
  }
  ...
  /**
   * Main method. See help message for more information about using this program. Entry point of the
   * program.
   * 
  ...
  public static void main(final String[] args) {
    ...
    try {
      // We use different method because it can be called outside main (for example, in from
      // ReportSonarPlugin)
      execute(args);
  ...
  public static void execute(final String[] args) throws BadExportationDataTypeException,
      BadSonarQubeRequestException, IOException, UnknownQualityGateException, OpenXML4JException,
      XmlException, SonarQubeException, ParseException {
    // Log message.
    String message;
    ...
    providerFactory =
        new StandaloneProviderFactory(url, conf.getToken(), conf.getProject(), conf.getBranch());


    // Initialize connection with SonarQube and retrieve primitive information
    ...
    final Report model = new ReportModelFactory(conf.getProject(), conf.getBranch(),
        conf.getAuthor(), conf.getDate(), providerFactory).create();
